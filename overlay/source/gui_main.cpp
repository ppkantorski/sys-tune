...
// VolumeTrackBar
//
// When volume == 0, draws the speaker glyph left-half only (via scissor) plus
// a mute cross.  The icon pointer is temporarily cleared so TrackBar::draw()
// skips its own icon draw; we then draw the icon ourselves under a tight scissor
// that covers only the left half — nothing else is ever clipped.
// =============================================================================
...
public:
    ...
    // -----------------------------------------------------------------------
    // Touch: tapping the speaker icon triggers mute/unmute, identical to Y.
    // The icon is drawn at getX()+42, baseline getY()+54, fontSize 30.
    // We use a generous hit area so the tap is easy to land.
    // -----------------------------------------------------------------------
    bool onTouch(tsl::elm::TouchEvent event, s32 currX, s32 currY,
        ...
        if (m_iconTapCallback) {
            // Hit area: X [getX()+30 .. getX()+75], Y [getY()+22 .. getY()+65]
            const bool inIcon =
            ...
            if (inIcon) {
                if (event == tsl::elm::TouchEvent::Release) {
                    // Only fire if the finger didn't wander far onto the slider.
                    ...
                    triggerNavigationFeedback();
                return true; // consume the event so the slider doesn't also react
            }
        }
    ...
    // Allows update() to relabel the slider when the running title changes.
